...
(function (global) {
  ...
  var TANK_DEF_HP = 0.55;
  var TANK_MIT_IDS = { shield_block: 1, demon_spikes: 1, elusive: 1 };
  ...
  function pressNow(actor, ab, target, tag) {
    var hold = _planning;
    _planning = false;
    var ok = false;
    try { ok = doCast(actor, ab, target, tag); } finally { _planning = hold; }
    return ok;
  }

  function pickTaunt(usable, kit, actor) {
    return pickReady(usable, kit.taunt, { allowFa: true, actor: actor, allowKick: true })
      || usable.find(function (a) { return a && a.type === 'taunt'; });
  }

  function foeHeldByTank(e, tank) {
    if (!e || !tank) return true;
    var taunted = (e.buffs || []).find(function (b) { return b && b.forceTarget; });
    if (taunted) {
      if (taunted.forceTarget === tank.uid) return true;
      return false;
    }
    var tgt = typeof getThreatTarget === 'function' ? getThreatTarget(e) : null;
    if (!tgt) return false;
    if (tgt.uid === tank.uid) return true;
    if (tgt.role === 'tank') return true;
    return false;
  }

  function tryTauntSwap(actor, usable, kit, friends, boss) {
    var taunt = pickTaunt(usable, kit, actor);
    ...
    if (other && otherStacks >= 2 && !iAmMt) {
      return isFreeAction(taunt) ? pressNow(actor, taunt, null, 'taunt') : doCast(actor, taunt, null, 'taunt');
    }
    return false;
  }

  function tryTauntLoose(actor, usable, kit, foes) {
    var taunt = pickTaunt(usable, kit, actor);
    if (!taunt) return false;
    var loose = (foes || []).some(function (e) { return e && e.alive && !foeHeldByTank(e, actor); });
    if (!loose) return false;
    return isFreeAction(taunt) ? pressNow(actor, taunt, null, 'taunt') : doCast(actor, taunt, null, 'taunt');
  }

  function hasOwnBuff(u, id) {
    return !!(u && (u.buffs || []).some(function (b) {
      return b && b.id === id && (b.turns == null || Number(b.turns) > 0);
    }));
  }

  function tryTankMit(actor, usable, kit, foes) {
    if (!(foes || []).length) return false;
    var ids = (kit.defensives || []).filter(function (id) { return TANK_MIT_IDS[id]; });
    if (!ids.length) return false;
    for (var i = 0; i < ids.length; i++) {
      if (hasOwnBuff(actor, ids[i])) continue;
      var ab = pickReady(usable, [ids[i]], { allowFa: true, actor: actor, target: actor, allowKick: true });
      if (!ab) continue;
      if (isFreeAction(ab)) {
        if (pressNow(actor, ab, actor, 'mit')) return true;
      } else if (doCast(actor, ab, actor, 'mit')) return true;
    }
    return false;
  }

  function tryTankBigSave(actor, usable, kit, foes) {
    var hp = hpRatio(actor);
    var incoming = hasBigIncoming(foes);
    if (hp >= TANK_DEF_HP && !(incoming && hp < 0.75)) return false;
    var big = (kit.defensives || []).filter(function (id) { return !TANK_MIT_IDS[id]; });
    if (!big.length) return false;
    var ab = pickReady(usable, big, { allowFa: true, actor: actor, target: actor, allowKick: true });
    if (!ab) return false;
    return isFreeAction(ab) ? pressNow(actor, ab, actor, 'save') : doCast(actor, ab, actor, 'save');
  }

  function tryTankDef(actor, usable, kit, foes) {
    if (tryTankMit(actor, usable, kit, foes)) return true;
    if (tryTankBigSave(actor, usable, kit, foes)) return true;
  ...
  function tryTank(actor, usable, kit, friends, foes, prio, boss) {
    tryTauntSwap(actor, usable, kit, friends, boss);
    tryTauntLoose(actor, usable, kit, foes);
    tryTankMit(actor, usable, kit, foes);
    tryTankBigSave(actor, usable, kit, foes);
    if (tryTauntSwap(actor, usable, kit, friends, boss)) return true;
    if (tryTauntLoose(actor, usable, kit, foes)) return true;
    if (tryTankDef(actor, usable, kit, foes)) return true;
