/* gear: slots, generation, equip UI */
  ...
  function generateGearItem(opts = {}) {
    ...
    // Primary split by slot + prefix/suffix
    ...
    // Вторички — живой рейтинг (stats.crit → critRating в applyGearToHero, 1:1).
    // 0.24: кусок +5 ~12–16 рейтинга, полный сет ~+40–80 к любимой вторичке.
    ...
    // скорость только сверху, не вместо крита/иск./унив.
    ...
    // Алиасы рейтинга = те же очки. sumGearStats читает crit, не складывает оба.
  ...
  function gearScore(it) {
    ...
    const s = it.stats;
    return readGearStat(s, 'atk') * 3 + readGearStat(s, 'hp') * 0.15 + readGearStat(s, 'def') * 2
      + readGearStat(s, 'crit', 'critRating') * 2
      + readGearStat(s, 'mastery', 'masteryRating') * 2
      + readGearStat(s, 'vers', 'versRating') * 2
      + readGearStat(s, 'speed') * 4
      + (it.ilvl || 0);
  }
  ...
  function sumGearStats(gear) {
    ...
    const aliases = { crit: 'critRating', mastery: 'masteryRating', vers: 'versRating' };
    for (const slot of GEAR_SLOT_IDS) {
      const it = g.equipped[slot];
      if (!it?.stats) continue;
      for (const k of Object.keys(tot)) tot[k] += readGearStat(it.stats, k, aliases[k]);
    }
    return tot;
  }
  ...
  /** Живой путь экипа: базы + шмот + силы ключа. Не трогает «голый» sec в лобби. */
