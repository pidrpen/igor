...
(function (global) {
      ...
      // Parts: builders gs → deploy_turret cs2 / siege cs3
      {
        ...
        abilities: [
          ...
          A({ id: 'rivet_gun', n: 'Заклёпочный пистолет', en: 'Rivet Gun', i: '🔫',
            c: 25, gs: 1, t: 'damage', fl: 30, school: 'physical',
            d: '30т · 25 пар · +1 деталь', sid: 90002 }),
          A({ id: 'plasma_cutter', n: 'Плазменный резак', en: 'Plasma Cutter', i: '✳️',
            c: 50, cs: 3, cd: 7, t: 'buff', petAtkMod: 0.5, bt: 4, school: 'none',
            d: '+50% урона пета · 4 хода · 50 пар · 3 детали · КД 7', sid: 90003 }),
          A({ id: 'bot_overdrive', n: 'Ускорение бота', en: 'Bot Overdrive', i: '⏩',
            c: 35, cs: 2, cd: 7, t: 'buff', bt: 4, school: 'none',
            d: 'Пар с атаки пета ×4 (5→20) · 4 хода · 35 пар · 2 детали · КД 7', sid: 90010 }),
          ...
          A({ id: 'emergency_repair', n: 'Аварийный ремонт', en: 'Emergency Repair', i: '🩹',
            c: 50, t: 'heal', school: 'none',
            d: 'Хил пета 20% его HP · 50 пар', sid: 90006 }),
          A({ id: 'call_siege_walker', n: 'Осадный ходун', en: 'Siege Walker', i: '🦾',
            c: 50, cs: 3, cd: 8, t: 'buff', petAtkMod: 0.7, bt: 4, school: 'none',
            d: 'Модуль: область +70% урона пета · 4 хода · 50 пар · 3 детали · КД 8', sid: 90007 }),
      ...
      // ST: remote_charge 26 · sticky DoT setup
      {
        ...
        abilities: [
          ...
          A({ id: 'shrapnel_blast', n: 'Шрапнель', en: 'Shrapnel Blast', i: '💥',
            c: 28, gs: 1, t: 'aoe', fl: 18, school: 'physical',
            d: '18т AoE · +1 деталь', sid: 90012 }),
          A({ id: 'cluster_bomb', n: 'Кассетная бомба', en: 'Cluster Bomb', i: '💣',
            c: 40, t: 'aoe', fl: 22, school: 'fire',
            d: '22т AoE · 40 пар', sid: 90013 }),
          ...
          A({ id: 'rocket_barrage', n: 'Ракетный залп', en: 'Rocket Barrage', i: '🚀',
            c: 45, cd: 2, t: 'aoe', fl: 24, school: 'fire',
            d: '24т AoE · КД 2', sid: 90015 }),
          A({ id: 'remote_charge', n: 'Дистанционный заряд', en: 'Remote Charge', i: '📡',
            c: 20, t: 'damage', fl: 38, school: 'fire',
            d: '38т · точечный подрыв', sid: 90016 }),
      ...
      // ─── Tinkerer (production unlocked) — keep intact ──────
      {
        ...
        abilities: [
          ...
          A({ id: 'rocket_chicken', n: 'Ракета-курица', en: 'Rocket Chicken', i: '🐔',
            c: 30, cs: 1, cd: 4, t: 'aoe', fl: 16, school: 'fire', d: '', sid: 90024 }),
  ...
  function validate() {
    const mech = ENGINEER_CLASS.specs.find((s) => s.id === 'mechanist');
    const sap = ENGINEER_CLASS.specs.find((s) => s.id === 'sapper');
    const byId = (spec, id) => spec.abilities.find((a) => a.id === id);
    const checks = {
      'no scrap_shot': !byId(mech, 'scrap_shot'),
      'no turret': !byId(mech, 'deploy_turret'),
      'rivet 30': byId(mech, 'rivet_gun') && byId(mech, 'rivet_gun').flat === 30,
      'plasma buff 50/3 cd7': byId(mech, 'plasma_cutter')
        && byId(mech, 'plasma_cutter').type === 'buff'
        && byId(mech, 'plasma_cutter').cost === 50
        && byId(mech, 'plasma_cutter').costSec === 3
        && byId(mech, 'plasma_cutter').cd === 7
        && byId(mech, 'plasma_cutter').petAtkMod === 0.5,
      'overdrive 35/2 cd7': byId(mech, 'bot_overdrive')
        && byId(mech, 'bot_overdrive').cost === 35
        && byId(mech, 'bot_overdrive').costSec === 2
        && byId(mech, 'bot_overdrive').cd === 7,
      'repair 50 no cd': byId(mech, 'emergency_repair')
        && byId(mech, 'emergency_repair').cost === 50
        && !byId(mech, 'emergency_repair').cd,
      'siege module not summon': byId(mech, 'call_siege_walker')
        && byId(mech, 'call_siege_walker').type === 'buff'
        && byId(mech, 'call_siege_walker').cd === 8
        && byId(mech, 'call_siege_walker').petAtkMod === 0.7,
      'wrench kept': !!byId(mech, 'wrench_bash'),
      'overclock kept': !!byId(mech, 'overclock'),
      'shock kept': !!byId(mech, 'shock_wrench'),
      'sapper untouched sticky': !!byId(sap, 'sticky_bomb'),
    };
