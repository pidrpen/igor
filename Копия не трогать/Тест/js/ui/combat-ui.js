/* ui/combat-ui: ability bar, unit cards, battle render */
  ...
  function onUnitClick(unit) {
    ...
    const isKick = isKickAbility(ability);
    if (rule === 'self_only') {
      pendingTarget = null;
  ...
  function stackHtml(u, actor, withPets) {
    ...
    return `<div class="unit-stack${extra}" data-uid="${u.uid}">${auraRailHtml(u)}${unitCard(u, actor)}${withPets ? petRowHtml(u, actor) : ''}</div>`;
  }

  function ensureCombatRowClicks() {
    if (ensureCombatRowClicks.done) return;
    ensureCombatRowClicks.done = true;
    const bind = (id, findUnit) => {
      ...
      row.addEventListener('click', (e) => {
        ...
        if (!card) return;
        const u = findUnit(card.dataset.uid);
        if (u) onUnitClick(u);
      });
  ...
  function auraSig(u) {
    const bits = (u.buffs || []).map(b => [b.id, b.stacks || 0, b.turns || 0, b.dot || 0, b._linked ? 1 : 0].join(':'));
  ...
  function unitClassName(u, actor) {
    ...
    return `unit ${u.side === 'ally' ? 'ally' : 'enemy'}${u.alive ? '' : ' dead'}${active ? ' active' : ''}${targeting ? ' selected-target' : ''}${castingCls}${low}${kickPrio}${focus}`;
  }
  ...
  function auraDetailLines(b, u) {
    ...
    if (b.dmgTakenMod) lines.push('Получает +' + Math.round(Number(b.dmgTakenMod) * 100) + '% урона');
  ...
  function raidAuraEntries(u) {
    ...
    if (u.thunderMark && !seen.has('storm_mark')) {
  ...
  function auraRailHtml(u) {
    ...
    const col = (list, cls, label) =>
      `<div class="aura-rail ${cls}" aria-label="${label}">` +
      list.slice(0, 3).map(b => auraIcoHtml(b, u)).join('') +
      (list.length > 3
        ? `<span class="aura-ico aura-more" data-tip-name="Ещё эффекты" data-tip-detail="${escAttr(list.slice(3).map(b => b.name || '').join(' | '))}">+${list.length - 3}</span>`
        : '') +
      '</div>';
    return `<div class="unit-aura">${col(good, 'aura-rail-good', 'Усиления')}${col(bad, 'aura-rail-bad', 'Ослабления')}</div>`;
  }
